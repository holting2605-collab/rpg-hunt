...
function normaliseState(s){
  window.RPG_HUNT_SKINS?.normalize(s);
...
function resetGame(){
  ...
  state=newState(); resumeCombatSession(); market=[]; ensureEmergencyRecruit(); rerollMarket(false); renderAll(); navStack=[]; navigate('hq',{},false);
}
...
function hunterCard(h, marketMode=false){
  ...
  return `<article class="hunter-card ${sel||active?'selected':''}" data-hunter="${h.id}">
    <button class="hunter-art hunter-art-button" data-openhunter="${h.id}" style="background-image:url('${portraitArt(h)}')" aria-label="${h.name} öffnen"><span class="tier-badge tier-${h.tier}">${h.tier}</span><span class="portrait-open">Details öffnen</span></button>
...
function renderHQ(){
  ...
  $('#hqTeam').innerHTML=team.length?team.map(h=>`<button class="mini-hunter mini-hunter-button" data-openhunter="${h.id}"><img loading="lazy" decoding="async" src="${portraitArt(h)}"><div><b>${h.name}</b><small>${h.tier} · L${h.level} · ${h.maxHp} HP</small></div></button>`).join(''):'<p style="color:var(--muted)">Noch kein Team ausgewählt.</p>';
...
function renderRoster(){
  if(!$('#roster')) return;
  $('#roster').innerHTML=state.roster.length?state.roster.map(h=>hunterCard(h,false)).join(''):'<div class="empty-state"><img loading="lazy" decoding="async" src="assets/hunter_2.jpg"><h3>Noch keine Hunter</h3><p>Öffne die Rekrutierung und wirb deinen ersten Hunter an.</p><button class="primary" data-nav="recruit">Zur Rekrutierung</button></div>';
}
function renderGearHub(){
  const box=$('#gearHunterList'); if(!box) return;
  box.innerHTML=state.roster.length?state.roster.map(h=>`<button class="portrait-select ${state.activeHunterId===h.id?'selected':''}" data-loadouthunter="${h.id}"><img loading="lazy" decoding="async" src="${portraitArt(h)}"><span><b>${h.name}</b><small>${h.tier} · Level ${h.level}</small><em>🔫 ${h.primary?.name||'leer'} · 🔸 ${h.sidearm?.name||'leer'} · 🎒 ${h.extras.length}/8</em></span></button>`).join(''):'<div class="empty-state"><p>Du hast noch keinen Hunter.</p><button class="primary" data-nav="recruit">Hunter rekrutieren</button></div>';
...
function renderHunterDetail(){
  ...
  box.innerHTML=`<div class="detail-shell">
    <div class="detail-portrait" style="background-image:url('${portraitArt(h)}')"><span class="tier-badge tier-${h.tier}">${h.tier}</span><div class="detail-name"><p>LEVEL ${h.level}</p><h1>${h.name}</h1><span>Wert ${moneyFmt(hunterValue(h))} $</span></div></div>
    <div class="detail-content">
      ...
      <div class="detail-grid-two">
        <section class="detail-panel"><p class="eyebrow">LOADOUT</p><h2>Ausrüstung</h2><button class="loadout-row" data-loadouthunter="${h.id}"><img loading="lazy" decoding="async" src="${h.primary?weaponArt(h.primary):'assets/rifle_2.jpg'}"><span><small>Primärwaffe</small><b>${h.primary?.name||'Nicht ausgerüstet'}</b></span><strong>›</strong></button><button class="loadout-row" data-loadouthunter="${h.id}"><img loading="lazy" decoding="async" src="${h.sidearm?weaponArt(h.sidearm):'assets/pistol_2.jpg'}"><span><small>Seitenwaffe</small><b>${h.sidearm?.name||'Nicht ausgerüstet'}</b></span><strong>›</strong></button><button class="loadout-row" data-loadouthunter="${h.id}"><img loading="lazy" decoding="async" src="assets/shotgun_2.jpg"><span><small>Extras</small><b>${h.extras.length}/8 ausgerüstet</b></span><strong>›</strong></button></section>
...
function renderLoadout(){
  ...
  const perks=h.perks.map(id=>D.PERKS[id]).filter(Boolean);
  box.innerHTML=`<div class="loadout-header"><img loading="lazy" decoding="async" src="${portraitArt(h)}"><div><p class="eyebrow">${h.tier} · LEVEL ${h.level}</p><h1>${h.name}</h1><p>Stelle das Loadout Schritt für Schritt zusammen. Abgerüstete Gegenstände landen im gemeinsamen Lager.</p></div></div>
...
function renderStashStrip(){
  ...
  if(armoryMode==='extras'){
    box.innerHTML=state.stashExtras.length?`<div class="stash-strip-title"><span>Im Lager</span><b>${state.stashExtras.length} Extras</b></div><div class="stash-items">${state.stashExtras.map((x,i)=>`<button class="stash-item" data-equipstashextra="${i}"><img loading="lazy" decoding="async" src="assets/pistol_2.jpg"><span><b>${x.name}</b><small>Aus Lager ausrüsten</small></span></button>`).join('')}</div>`:'<div class="stash-strip-empty">Keine Extras im Lager.</div>';
  } else {
...
function offerPerks(title,onDone){
  ...
  if(state.run)state.run.pendingPerk={title,recipient:recipient.id,options:opts.map(p=>p.id)};
  modalLock=true; openModal(`<p class="eyebrow">TRAIT-SPUR</p><h2>${title}</h2><p>${recipient.name} erhält einen Trait. Maximal 15 Traits, 24 Gewicht.</p><div class="perk-options">${opts.map(p=>`<button class="perk-card" data-perkpick="${p.id}"><span class="perk-rarity">${p.rarity} · Gewicht ${p.weight}</span>${traitIcon(p)}<h3>${traitName(p)}</h3><p>${p.text}</p></button>`).join('')}</div>`); window._perkDone=onDone; window._perkRecipient=recipient.id;
}
function selectPerk(id){ const h=state.roster.find(x=>x.id===window._perkRecipient); const p=D.PERKS[id]; if(h&&p&&canAddPerk(h,p)){h.perks.push(p.id);log(`${h.name} erhält Trait: ${p.name}.`,'log-gold');} if(state.run)delete state.run.pendingPerk; modalLock=false; $('#modal').classList.add('hidden'); const cb=window._perkDone; window._perkDone=null; cb?.(); renderRun(); maybeSave(); }
...
function renderCodex(){ const box=$('#codexContent'); const cards={
 ...
 perks:D.PERKS.map(p=>`<article class="codex-card">${traitIcon(p)}<h3>${traitName(p)}</h3><div class="line"><span>Stufe</span><b>${p.rarity}</b></div><div class="line"><span>Gewicht</span><b>${p.weight}</b></div><p>${p.text}</p></article>`).join(''),
...
function hunterCard(h,marketMode=false){
  ...
  const maxA=100; return `<article class="hunter-card ${sel?'selected':''}" style="--tier:${tierColor(h.tier)}">
    <button class="hunter-portrait" data-openhunter="${h.id}" aria-label="${h.name} öffnen"><img loading="lazy" decoding="async" src="${portraitArt(h)}" alt=""><span class="tier-chip">${h.tier}</span>${sel?'<span class="team-check">✓</span>':''}</button>
...
function renderHQ(){
  const team=state.selectedTeam.map(id=>state.roster.find(h=>h.id===id)).filter(Boolean);
  $('#hqTeam').innerHTML=team.length?team.map(h=>`<button class="team-mini" data-openhunter="${h.id}"><img loading="lazy" decoding="async" src="${portraitArt(h)}" alt=""><span><b>${h.name}</b><small>${h.tier} · ${h.role} · L${h.level}</small><small>${h.primary?.name||'keine Primärwaffe'}</small></span></button>`).join(''):'<div class="empty-state">Noch kein Missionsteam ausgewählt.</div>';
...
function renderTraits(){const h=activeHunter();if(!h){$('#traitBoard').innerHTML='<div class="empty-state">Kein Hunter ausgewählt.</div>';return;}$('#traitBuyer').textContent=`${h.name} · ${h.upgradePoints||0} Upgrade-Punkte · ${h.perks.length}/15 Traits`;const q=($('#traitSearch')?.value||'').trim().toLowerCase(),rar=$('#traitRarity')?.value||'';let list=D.PERKS.filter(p=>(!q||traitName(p).toLowerCase().includes(q)||p.text.toLowerCase().includes(q))&&(!rar||p.rarity===rar));$('#traitBoard').innerHTML=list.map(p=>{const owned=h.perks.includes(p.id),cost=perkCost(p),buyable=p.rarity!=='mythic';return `<article class="trait-card ${owned?'owned':''}" style="--rarity:${p.rarity==='leicht'?'#6f9475':p.rarity==='mittel'?'#6f8fa5':p.rarity==='schwer'?'#9a73ad':'#c59a49'}">${traitIcon(p)}<h3>${traitName(p)}</h3><p>${p.text}</p><div class="trait-cost"><span>${p.rarity} · Gewicht ${p.weight}</span>${owned?'<b>Gelernt</b>':buyable?`<button class="btn secondary tiny" data-buytrait="${p.id}">${cost} UP</button>`:'<b>Nur im Run</b>'}</div></article>`;}).join('');}
...
function importSaveFile(file){
  ...
  reader.onload=()=>{const previous=state;try{
    const candidate=JSON.parse(reader.result);
    if(!candidate||!Number.isFinite(candidate.money)||candidate.money<0||!Array.isArray(candidate.roster)||!Array.isArray(candidate.selectedTeam))throw new Error('Invalid save');
    if(candidate.roster.some(h=>!h||typeof h.id!=='string'||typeof h.name!=='string'||!Array.isArray(h.extras)||!Array.isArray(h.perks)||!Number.isFinite(h.maxHp)))throw new Error('Invalid hunter');
    const loaded=normaliseState(candidate);
    if(loaded.run&&(!Array.isArray(loaded.run.teamIds)||!Array.isArray(loaded.run.log)||!Number.isFinite(loaded.run.round)))throw new Error('Invalid run');
    if(loaded.run?.combat&&!Array.isArray(loaded.run.combat.enemies))throw new Error('Invalid combat');
    state=loaded; resumeCombatSession(); renderAll(); navigate(state.run?'mission':'hq',{},false); restorePendingPerk(); saveState(true); toast('Savegame importiert.');
  }catch{if(state!==previous){state=previous;resumeCombatSession();renderAll();restorePendingPerk();}toast('Ungültiges Savegame. Der bisherige Spielstand bleibt erhalten.');}};
...
function renderShop(){
  ...
  $('#shopBuyerBox').innerHTML = buyer ? `<button class="shop-buyer" data-loadouthunter="${buyer.id}"><img loading="lazy" decoding="async" src="${portraitArt(buyer)}" alt=""><span><small>Aktiver Käufer</small><b>${buyer.name}</b><em>${buyer.tier} · ${buyer.role} · ${moneyFmt(hunterValue(buyer))} $ Wert</em></span></button>` : `<div class="shop-buyer empty"><span><small>Kein Hunter</small><b>Erst rekrutieren</b></span></div>`;
...
function renderShopWeapons(box,buyer){
  ...
  box.innerHTML=`<section class="weapon-carousel-shell">
    ...
    <div class="carousel-side"><img loading="lazy" decoding="async" src="${weaponArt(prev)}"><span>${prev.name}</span></div>
    <article class="featured-weapon" style="--art:url('${weaponArt(w)}')">
      <div class="featured-image"><img loading="lazy" decoding="async" src="${weaponArt(w)}" alt=""></div>
    ...
    <div class="carousel-side"><img loading="lazy" decoding="async" src="${weaponArt(next)}"><span>${next.name}</span></div>
  ...
  <div class="shop-quick-grid">${list.slice(0,18).map((x,i)=>`<button class="quick-weapon ${x.name===w.name?'active':''}" data-shop-jump="${i}"><img loading="lazy" decoding="async" src="${weaponArt(x)}"><span>${x.name}</span><small>${moneyFmt(x.price)} $</small></button>`).join('')}</div>`;
}
...
function renderShopExtras(box,buyer){
  ...
  box.innerHTML=`<div class="shop-grid-head"><div><p class="eyebrow">TOOLS & CONSUMABLES</p><h2>Vorräte kaufen</h2><p>Medikits, Bomben, Fallen, Messer, Wurfäxte, Beetles und Spritzen.</p></div><img loading="lazy" decoding="async" src="${V.menuArt.extrasWorkbench||V.extraArt.med}" alt=""></div><div class="merchant-grid">${list.map((x,i)=>`<article class="merchant-card"><img loading="lazy" decoding="async" src="${extraArt(x)}"><div><p class="eyebrow">${x.type==='tool'?'Tool':'Consumable'} · ${x.uses===99?'dauerhaft':x.uses+'x'}</p><h3>${x.name}</h3><p>${x.text}</p><div class="weapon-stats">${x.damage?`<span>DMG ${x.damage}</span>`:''}${x.heal?`<span>HEAL ${x.heal}</span>`:''}${x.status?`<span>${x.status}</span>`:''}</div></div><div><b class="price">${moneyFmt(x.price)} $</b><button class="btn primary tiny" data-buyextra="${D.EXTRAS.indexOf(x)}" ${buyer?'':'disabled'}>Kaufen</button><button class="btn secondary tiny" data-shop-extra-stash="${D.EXTRAS.indexOf(x)}">Ins Lager</button></div></article>`).join('')}</div>`;
}
function renderShopSell(box,buyer){
  const w=state.stashWeapons, e=state.stashExtras;
  box.innerHTML=`<div class="shop-grid-head"><div><p class="eyebrow">LAGER & VERKAUF</p><h2>Alte Beute zu Geld machen</h2><p>Verkaufspreis: 55 % vom Händlerwert. Ausgerüstete Gegenstände erst im Loadout abrüsten.</p></div><img loading="lazy" decoding="async" src="${V.menuArt.merchant||V.menuArt.shop||V.menuArt.gear}" alt=""></div>
  <div class="sell-columns"><section class="panel"><h2>Waffen im Lager (${w.length})</h2>${w.length?w.map((x,i)=>`<div class="sell-row"><img loading="lazy" decoding="async" src="${weaponArt(x)}"><span><b>${x.name}</b><small>${x.category} · DMG ${x.damage}/${x.damageMax}</small></span><strong>${moneyFmt(sellValue(x.price))} $</strong><button class="btn tiny secondary" data-shop-equip-stash-w="${i}" ${buyer?'':'disabled'}>Ausrüsten</button><button class="btn tiny danger" data-shop-sellw="${i}">Verkaufen</button></div>`).join(''):'<p class="muted">Keine Waffen im Lager.</p>'}</section>
  <section class="panel"><h2>Extras im Lager (${e.length})</h2>${e.length?e.map((x,i)=>`<div class="sell-row"><img loading="lazy" decoding="async" src="${extraArt(x)}"><span><b>${x.name}</b><small>${x.type} · ${x.currentUses===99?'∞':x.currentUses} Einsatz</small></span><strong>${moneyFmt(sellValue(x.price))} $</strong><button class="btn tiny secondary" data-shop-equip-stash-e="${i}" ${buyer?'':'disabled'}>Ausrüsten</button><button class="btn tiny danger" data-shop-selle="${i}">Verkaufen</button></div>`).join(''):'<p class="muted">Keine Extras im Lager.</p>'}</section></div>`;
}
...
function renderCombat(){
  ...
  $('#combatPanel').innerHTML=`<div class="combat-sim-shell ${state.settings.motion===false?'no-motion':''}">
    ...
    <div class="battle-theatre" style="--battle-bg:url('${env.art}')">
      ...
      <div class="actor-lane team-lane">${livingTeam().map((h,i)=>{ensureHunterAI(h);const w=c.weaponState?.[h.id]||aiPickWeapon(h,c);return `<article class="battle-actor hunter-actor pos-${i}" data-stage-actor="${h.id}"><img loading="lazy" decoding="async" src="${portraitArt(h)}" alt=""><div class="actor-info"><strong>${h.name}</strong><small>${h.ai.profile} · KI ${aiGrade(h.ai.intelligence)}</small><em data-weapon>${w?.name||'Fäuste'}</em><div class="actor-hp"><i style="width:${clamp(h.hp/h.maxHp*100,0,100)}%"></i></div><span>🫁 <b data-stamina>${h.stamina}/${h.maxStamina}</b></span></div></article>`;}).join('')}</div>
      <div class="battle-center-mark"><span>${distanceLabel(c.distance)}</span><i></i></div>
      <div class="actor-lane enemy-lane">${c.enemies.filter(e=>e.hp>0).map((e,i)=>`<article class="battle-actor enemy-actor pos-${i}" data-stage-enemy="${e.id}"><img loading="lazy" decoding="async" src="${enemyArtPlayer(e)}" alt=""><div class="actor-info"><strong>${e.name}</strong><small>${e.kind==='boss'?'TARGET':e.kind==='hunter'?'FEINDLICHER HUNTER':'KREATUR'}</small><div class="actor-hp"><i style="width:${clamp(e.hp/e.maxHp*100,0,100)}%"></i></div><span>${Math.max(0,e.hp)}/${e.maxHp} HP</span></div></article>`).join('')}</div>
    ...
    <div class="ai-roster-strip">${livingTeam().map(h=>`<div><img loading="lazy" decoding="async" src="${portraitArt(h)}" alt=""><span><b>${h.name}</b><small>${aiSummary(h)}</small></span><em>🎯 ${h.accuracy} · 🍀 ${h.luck} · 🫁 ${h.stamina}</em></div>`).join('')}</div>
  </div>`;
...
async function runAutoCombatRound(){
  const r=state.run,c=r?.combat;if(!c||playerCombatBusy)return; playerCombatBusy=true; const token=c.token; const currentCombat=()=>state.run===r&&r.combat===c&&c.token===token;
  ...
  for(const h of [...livingTeam()]){
    ...
    if(!prev||prev.name!==nextW.name){c.weaponState[h.id]=nextW;setAiAction(`${h.name} wechselt auf ${nextW.name}`,`${h.ai.profile}: Distanz ${distanceLabel(c.distance)} wird neu bewertet.`);const a=stageActor(h.id);if(a){a.querySelector('[data-weapon]').textContent=nextW.name;await pulseNode(a,'weapon-switch',300);if(!currentCombat())return;}log(`${h.name} wechselt selbstständig auf ${nextW.name}.`);}
    const w=c.weaponState[h.id]||nextW;const action=aiDecideAction(h,w,c);
    if(action.type==='heal'){setAiAction(`${h.name} benutzt ${action.extra.name}`,'Die KI priorisiert Überleben vor einem weiteren Schuss.');await pulseNode(stageActor(h.id),'heal-action',380);if(!currentCombat())return;aiUseExtra(h,action.extra);syncBattleHUD();await waitAI(260);if(!currentCombat())return;continue;}
    if(action.type==='recover'){const gain=Math.min(3,h.maxStamina-h.stamina);h.stamina+=gain;setAiAction(`${h.name} holt Luft`, `+${gain} Ausdauer – ${h.ai.profile} wartet auf einen besseren Moment.`);await pulseNode(stageActor(h.id),'recover-action',420);if(!currentCombat())return;log(`${h.name} regeneriert ${gain} Ausdauer.`,'log-good');syncBattleHUD();continue;}
    ...
    const a=stageActor(h.id),t=stageEnemy(target.id);await pulseNode(a,action.type==='rush'?'rush-action':'fire-action',340);if(!currentCombat())return;
    if(chance(p)){
      ...
      await pulseNode(t,'hit-action',320);if(!currentCombat())return;log(`${h.name} trifft ${target.name} mit ${w.name}: ${dmg} Schaden.`,'log-good');playSound('shot');
    }else{await pulseNode(t,'miss-action',220);if(!currentCombat())return;log(`${h.name} verfehlt ${target.name}.`);}
    r.attention=clamp(r.attention+Math.max(0,(w.noise||3)-3),0,100);syncBattleHUD();await waitAI(170);if(!currentCombat())return;
  }
  if(!currentCombat())return;
  for(const e of c.enemies.filter(e=>e.hp>0)){
    const targets=livingTeam();if(!targets.length)break;const h=pick(targets);const en=stageEnemy(e.id),hn=stageActor(h.id);setAiAction(`${e.name} greift ${h.name} an`,'Der Gegner reagiert auf eure Position.');await pulseNode(en,'enemy-attack',300);if(!currentCombat())return;
    const dodge=clamp(.12+(h.speed-5)*.02+(h.luck||0)*.004,.05,.34);
    if(chance(dodge)){await pulseNode(hn,'dodge-action',260);if(!currentCombat())return;log(`${h.name} weicht ${e.name} aus.`);}
    else{let dmg=rnd(Math.max(1,e.attack-1),e.attack+1);const mods=hunterMods(h);if(e.kind==='monster'||e.kind==='boss')dmg=Math.round(dmg*(1-(mods.meleeResist||0)));h.hp=Math.max(0,h.hp-dmg);await pulseNode(hn,'hit-action',320);if(!currentCombat())return;log(`${e.name} trifft ${h.name}: ${dmg} Schaden.`,'log-bad');if(e.status&&chance(.35))applyStatus(h,e.status);}
    syncBattleHUD();await waitAI(170);if(!currentCombat())return;
  }
  livingTeam().forEach(h=>tickStatuses(h,true));c.turn++; syncBattleHUD(); maybeSave();
...
function restorePendingPerk(){
  const pending=state.run?.pendingPerk;if(!pending)return;
  const h=state.roster.find(h=>h.id===pending.recipient),opts=pending.options.map(id=>D.PERKS[id]).filter(Boolean);
  if(!h||!opts.length){delete state.run.pendingPerk;return;}
  modalLock=true;window._perkRecipient=h.id;window._perkDone=()=>advanceRound();
  openModal('<p class="eyebrow">TRAIT-SPUR</p><h2>Wähle deine Verbesserung</h2><p>'+h.name+' · 15 Traits / 24 Gewicht</p><div class="perk-options">'+opts.map(p=>'<button class="perk-card" data-perkpick="'+p.id+'"><span>'+p.rarity+' · Gewicht '+p.weight+'</span>'+traitIcon(p)+'<h3>'+traitName(p)+'</h3><p>'+p.text+'</p></button>').join('')+'</div>');
}
function resumeCombatSession(){
  modalLock=false;window._perkDone=null;$('#modal').classList.add('hidden');
  clearTimeout(playerCombatTimer); playerCombatBusy=false; playerCombatToken++;
  if(state.run?.combat){state.run.combat.token=playerCombatToken;state.run.combat.weaponState=state.run.combat.weaponState||{};}
}

// v0.6 UI adapter: keep gameplay functions and save schema in this closure.
const playerUI = window.RPG_HUNT_UI({
  get state(){return state;}, get market(){return market;},
  get context(){return navContext;}, set context(value){navContext=value;},
  get armoryMode(){return armoryMode;}, set armoryMode(value){armoryMode=value;},
  activeHunter, portraitArt, weaponArt, extraArt, weaponSlot, weaponFilters, shopInitFilters,
  shopWeaponList, aiBehaviourText, canAddPerk, perkCost, ensureContract, sellValue,
  rerollMarket, toast, openModal, closeModal, buyExtra, unequipExtra,
  equipStashExtra, sellStashWeapon, sellStashExtra, save:()=>saveState(true)
}, {renderHQ,renderHunterDetail,renderGearHub,renderTraits,renderMission,renderCodex,renderSettings});
renderHQ=playerUI.renderHQ; renderRecruitment=playerUI.renderRecruitment;
renderRoster=playerUI.renderRoster; renderHunterDetail=playerUI.renderHunterDetail;
renderLoadout=playerUI.renderLoadout; renderArmory=playerUI.renderArmory;
renderShop=playerUI.renderShop; renderTraits=playerUI.renderTraits;
renderMission=playerUI.renderMission; hunterCard=playerUI.hunterCard; previewRecruit=playerUI.previewRecruit;
navigate=playerUI.navigate; goBack=playerUI.back;
const renderPlayerBase=renderAll;
renderAll=function(){renderPlayerBase();playerUI.refresh();};
...
const bootView=new URLSearchParams(location.search).get('view'); navigate(state.run?'mission':bootView&&document.querySelector(`#screen-${bootView}`)?bootView:'hq',{},false);
restorePendingPerk();
